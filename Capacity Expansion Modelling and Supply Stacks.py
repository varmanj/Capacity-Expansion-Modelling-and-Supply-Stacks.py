...
# small enough tolerance for 1 result (found this manually - trial and error)
...
# need to adjust bar width for volume (MW)
...
# plot 50% solar hour
# getting the bars to be adjacent to each other proved to be very challenging
# the width does in fact correspond to the fractional output of each source, but are a fraction of the space each bar is
# allocated. You would have to shift the bars manually based on how much space is between them. This will be taken care
# of post-production.

fig, ax1 = plt.subplots(figsize=(10, 6), sharex=True)
fig.subplots_adjust(hspace=0.05)

ax1.bar(x=supply_stack['Generator'], height=supply_stack['Marginal Cost'],
        width=fractional_output_at_50_percent_solar.values,
        color=[n.carriers.loc[gen, 'color'] for gen in supply_stack['Generator']])

ax1.set_ylabel('Marginal Cost (€/MWh)')
ax1.set_yscale('log')
ax1.set_xlabel('Generator')
ax1.set_title(f'Supply Stack at {hour_50_percent_solar}')
...
solar_stack = supply_stack.iloc[0]

ax1.bar(x=solar_stack['Generator'], height=solar_stack['Marginal Cost'],
        color=[n.carriers.loc[solar_stack["Generator"], 'color']])

ax1.set_ylabel('Marginal Cost (€/MWh)')
#ax1.set_yscale('log')
ax1.set_xlabel('Generator')
ax1.set_title(f'Supply Stack at {max_solar_hour}')
...
ocgt_stack = supply_stack.iloc[3]

ax1.bar(x=ocgt_stack['Generator'], height=ocgt_stack['Marginal Cost'],
        color=[n.carriers.loc[ocgt_stack["Generator"], 'color']])

ax1.set_ylabel('Marginal Cost (€/MWh)')
#ax1.set_yscale('log')
ax1.set_xlabel('Generator')
ax1.set_title(f'Supply Stack at {max_ocgt_hour}')

plt.tight_layout()
plt.show()

# GW/generator by hour for the day of interest

# first, day with the hour with 100% solar contribution found earlier

# Define the time range
# 100% solar @ apr12 noon
start_time = '2015-04-12 01:00:00'
end_time = '2015-04-12 16:00:00'

fig, ax = plt.subplots()

# Get the DataFrame
df = n.statistics.supply(comps=["Generator"], aggregate_time=False).droplevel(0)

df_selected = df.loc[['OCGT', 'solar', 'onwind', 'offwind'], start_time:end_time].div(1e3)
...
df_transposed.plot(
    kind='area',
    title="Maximum Solar Hour in GW Output by Generator",
    ax=ax,
    legend=True,
    linewidth=0,
)
ax.legend(bbox_to_anchor=(1, 0), loc="lower left", title="Carriers", ncol=1)
plt.show()
...
# Define the time range
# OCGT max @ feb 25 8pm
# range set to have desired hour visible (trial and error...)
start_time = '2015-02-25 05:00:00'
end_time = '2015-02-26 04:00:00'
...
df_transposed.plot(
    kind='area',
    title="Maximum OCGT Hour in GW Output by Generator",
    ax=ax,
    legend=True,
    linewidth=0,
)
...
# Define the time range
# 50% solar @ sep 7 12pm
start_time = '2015-09-07 02:00:00'
end_time = '2015-09-07 17:00:00'
...
df_transposed.plot(
    kind='area',
    title="50% Solar Hour in GW Output by Generator",
    ax=ax,
    legend=True,
    linewidth=0,
)
